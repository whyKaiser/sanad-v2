import type {IntakeRecord,DirectiveRecord} from '../lib/sanad/operations';
...
async function newRequest() {
  ...
  const response = await request('/api/intake', 'POST', form()); assert.equal(response.status, 201, await response.clone().text()); return { row: await response.json() as IntakeRecord, metadata, form, bytes };
}
async function step(row: IntakeRecord, name: string) { const r = await request(`/api/intake/${row.id}/${name}`, name === 'review' ? 'PATCH' : 'POST', { revision: row.revision, ...(name === 'review' ? { fields, reviewNote: 'راجعت النسخة الاصطناعية وحقولها يدويًا' } : {}) }); assert.equal(r.status, 200, await r.clone().text()); return r.json() as Promise<IntakeRecord>; }
...
test('Pre-arrival persists a real file before a case exists, protects sequencing, and replays safely', async () => {
  const before: {cases:CaseRecord[]} = await (await request('/api/state')).json(), fixture = await newRequest(); let row = fixture.row;
  assert.ok(!JSON.stringify(row).includes('objectKey')); assert.equal(row.status, 'saved'); assert.equal(row.caseId, null);
  assert.equal((await (await request('/api/state')).json() as {cases:CaseRecord[]}).cases.length, before.cases.length);
  ...
  const replay = await request(`/api/intake/${row.id}/arrival`, 'POST', arrival(row.revision)); assert.equal(replay.status, 200); assert.equal((await replay.json() as CaseRecord).id, record.id);
  assert.equal((await request(`/api/intake/${row.id}/arrival`, 'POST', { ...arrival(row.revision), port: 'منفذ آخر' })).status, 409);
  assert.equal((await (await request('/api/state')).json() as {cases:CaseRecord[]}).cases.length, before.cases.length + 1);
...
test('New workflows reject unauthenticated users, foreign records, forged data, and foreign origins', async () => {
  ...
  assert.equal((await request('/api/intake', 'POST', form({ ...metadata, requestKey: crypto.randomUUID(), synthetic: false }))).status, 400);
...
test('Incoming directive drafts can fix missing document links and freeze both travel documents', async () => {
  const state: {cases:CaseRecord[]} = await (await request('/api/state')).json(), r: CaseRecord = state.cases.find((r: CaseRecord) => r.reference === 'SND-2026-0004')!; assert.ok(r);
  ...
  const created = await request(`/api/cases/${r.id}/directives`, 'POST', input); assert.equal(created.status, 201); let order: DirectiveRecord = await created.json();
