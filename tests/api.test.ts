import type {CaseRecord,StoredDocument,Answer,AuditEvent} from '../lib/sanad/types';
...
test('Document lifecycle, duplicate, stale edit, packet snapshot, and persistence',async()=>{
 ...
 const created=await request('/api/cases','POST',{...input,name:'حالة اختبار آلي',notes:'SANAD AUTOMATED TEST'});assert.equal(created.status,201);const record:CaseRecord=await created.json();
 assert.equal((await request(`/api/packet/${record.id}`,'POST',{})).status,400);
 const missing:Answer=await (await request('/api/assistant','POST',{caseId:record.id,query:'اعرض الجواز'})).json();assert.match(missing.text,/لا يمكن استعادة/);
 ...
 const added=await request('/api/documents','POST',form(png));assert.equal(added.status,201);const doc:StoredDocument=await added.json();assert.equal((await request('/api/documents','POST',form(png))).status,409);
 ...
 const review={fields,reviewStatus:'approved',reviewNote:'مراجعة بيانات عينة آلية',revision:1};const reviewed=await request(`/api/documents/${doc.id}`,'PATCH',review);assert.equal(reviewed.status,200);assert.equal((await reviewed.json() as CaseRecord).status,'ready');
 assert.equal((await request(`/api/documents/${doc.id}`,'PATCH',review)).status,409);
 const packet=await request(`/api/packet/${record.id}`,'POST',{});assert.equal(packet.status,200);const saved:{id:string}=await packet.json();
 const before=await (await request(`/api/packet-snapshots/${saved.id}`)).text();assert.match(before,/سُجلت مراجعة هذه الحزمة/);
 assert.equal((await request(`/api/cases/${record.id}`,'PATCH',{notes:'<script>window.XSS=1</script>',consularStatus:'under_review',revision:(await (await request(`/api/cases/${record.id}`)).json() as {revision:number}).revision})).status,200);
 ...
 const state:{cases:CaseRecord[];audit:AuditEvent[]}=await (await request('/api/state')).json();assert.ok(state.cases.some((x)=>x.id===record.id&&x.documents.length===1));assert.ok(state.audit.some((x)=>x.caseId===record.id&&x.action==='packet_reviewed'));
