import type {Answer} from '../lib/sanad/types';
...
test("Profile persists with revision control and preserves independent case notes", async () => {
  ...
  assert.equal((await request(`/api/cases/${r.id}`, 'PATCH', { notes: 'ملاحظة مستقلة للاختبار', consularStatus: 'under_review', revision:r.revision })).status, 200);
...
test("Travel persists, prevents duplicate references, blocks links outside the case, and cites the exact movement", async () => {
  ...
  const answer: Answer = await (await request('/api/assistant', 'POST', { caseId: r.id, query: 'اعرض آخر دخول وخروج' })).json(); assert.equal(answer.mode, 'direct'); assert.equal(answer.evidence[0].movementId, movementId);
...
test("Concurrent first saves cannot overwrite one another", async () => {
  ...
  const saved: CaseRecord = await (await request(`/api/cases/${r.id}`)).json(); assert.equal(saved.details!.revision, 1);
});
test("Reviewed packet freezes added profile and travel data despite later edits", async () => {
  const state: {cases:CaseRecord[]} = await (await request('/api/state')).json();
  ...
  const saved = await request(`/api/packet/${r.id}`, 'POST', {}); assert.equal(saved.status, 200); const snapshot: {id:string} = await saved.json();
