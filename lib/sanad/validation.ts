...
export const caseUpdateSchema=z.object({notes:z.string().trim().max(2000),consularStatus:z.enum(["not_started","under_review","document_issued","additional_info"]),revision:z.number().int().positive()}).strict();
export const assistantSchema=z.object({caseId:z.string().min(1).max(80),query:z.string().trim().min(2).max(700),intent:z.enum(["passport","differences","replacement","summary","nationality","source"]).nullable().optional(),semantic:z.boolean().optional(),provider:z.enum(["groq","local"]).optional()}).strict();
