...
import { Plus, Minus, Pencil, Loader2, UserRound, BookOpen, Ticket, ChevronDown } from "lucide-react";
...
import type { Role } from "@/lib/sanad/integrated";
...
export default function TravelerPanel({ role, record, onRefresh }: { role: Role; record: CaseRecord; onRefresh: () => Promise<void> }) {
  const {canEdit}=permissionsFor(role);
  ...
  function edit() { if(!canEdit)return; setProfile(structuredClone(details.profile)); setRevision(details.revision); setError(""); setOpen(true); }
  ...
  return <div className="traveler-panel"><div className="traveler-toolbar"><div><h2>معلومات المسافر</h2><p>خانات السجل مع نسخ الوثائق ومصادرها في ملف واحد.</p></div>{canEdit&&<Button variant="outline" onClick={edit}><Pencil size={16}/>تعديل الخانات</Button>}</div>
    {profileGroups.map((g, i) => { const Icon = icons[i]; const content = <InfoGrid rows={[...core[i], ...g.fields.map(([k, label]) => [label, details.profile.values[k]]), ...g.dates.map(([k, label]) => [label, details.profile.dates[k].value ? displayRecordedDate(details.profile.dates[k]) : ""])]}/>; return i === 3 ? <details className="traveler-section" key={g.title}><summary><span className="expand-mark" aria-hidden="true"><Plus className="when-closed" size={18}/><Minus className="when-open" size={18}/></span>{g.title}<span>فتح / إغلاق التفاصيل</span></summary>{content}</details> : <section className="traveler-section" key={g.title}><h3><Icon size={18}/>{g.title}</h3>{content}</section>; })}
